...
开发过程中可以先运行相关测试；交付前运行**统一验证入口**（语法 + diff-check + 架构检查 + 全量 pytest）：

```bash
python -m tools.verify          # full：完整交付验证（含全量 pytest）
python -m tools.verify --fast   # fast：仅静态检查
```

架构依赖规则由 `tools/arch_check.py` 机械化执行（规则来源即本文件；历史债务登记在 `tools/arch_check_baseline.json`，只允许保留、不得增长）。新增业务依赖前先阅读本文件的「允许的依赖方向」与「禁止的典型依赖」。

如果未运行完整验证，必须在交付说明中明确记录未运行项和原因。纯文档任务经明确授权可以不运行验证。不得通过删除断言、放宽测试或跳过真实失败制造绿灯。
