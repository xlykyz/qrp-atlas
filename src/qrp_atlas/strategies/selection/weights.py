...
from .selection import RANK_COLUMN, SCORE_COLUMN, SELECTED_COLUMN, SelectionError
...
def equal_weight_targets(
    ...
    *,
    ...
    ordered = sorted({str(asset_id) for asset_id in selected_assets if str(asset_id).strip()})
    ...
    for asset_id in ordered:
        if scores is not None and asset_id in scores:
            priority = float(scores[asset_id])
        elif ranks is not None and asset_id in ranks:
...
def selection_to_target_weights(
    ...
    *,
    ...
    previous_assets_by_trade_date:
        Optional prior holdings used to emit zero targets for exits when the
        previous selection is not itself present in ``selection``.
    ...
    if trade_date_column is not None:
        ...
        if TRADE_DATE not in working.columns:
            raise WeightConstructionError("selection must include trade_date/signal_date")
    ...
    if previous_assets_by_trade_date:
        # Flatten only used when no chronological previous state is available.
        pass
    ...
    for execution_date in ordered_dates:
        day = working[working[execution_col].map(normalize_trade_date) == execution_date]
        selected = day[day[SELECTED_COLUMN].astype(bool)]
    ...
    if not rows:
        return pd.DataFrame(columns=list(TARGET_WEIGHT_COLUMNS))
    out = pd.DataFrame(rows, columns=list(TARGET_WEIGHT_COLUMNS))
    out = out.sort_values(["trade_date", "asset_id"], kind="mergesort").reset_index(drop=True)
    return out


def _as_signal_trade_map(signal_to_trade: Mapping[Any, Any] | pd.DataFrame) -> dict[pd.Timestamp, pd.Timestamp]:
    if isinstance(signal_to_trade, pd.DataFrame):
        ...
        for row in signal_to_trade.itertuples(index=False):
            mapping[normalize_trade_date(row.signal_date)] = normalize_trade_date(row.trade_date)
        return mapping
