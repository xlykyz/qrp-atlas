...
class HistoricalUniverseRequest:
    ...
    def normalized_dates(self) -> list[Any]:
        return list(self.trade_dates)


def _normalize_dates(trade_dates: Sequence[Any] | Any) -> list[Any]:
    if trade_dates is None:
        return []
    if isinstance(trade_dates, (str, bytes)) or not isinstance(trade_dates, Sequence):
        return [trade_dates]
...
def _normalize_assets(asset_ids: Sequence[str] | None) -> list[str]:
    ...
    if isinstance(asset_ids, str):
        ...
        items = [str(a) for a in asset_ids]
...
def build_historical_universe(
    ...
    *,
    ...
    Args:
        trade_dates: one date or a sequence of dates.
    ...
    Returns:
        ...
        stably by both. Index universes also retain ``index_code`` and
        ``snapshot_date`` when available.
    """
        ...
        piece = pd.DataFrame(
            {
                ...
                ASSET_ID: components[ASSET_ID].astype(str),
            }
        )
    ...
    # de-duplicate within a date while preserving first-seen order from query
