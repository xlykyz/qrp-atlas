...
from qrp_atlas.pipeline.pit_backfill.runner import RUN_TAG, BackfillConfig, PitBackfillRunner, parse_stages
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--stages",
        default="all",
        help="comma list or all: fetch,clean,load (decoupled stages)",
    )
    p.add_argument("--resume", action="store_true", help="resume from manifest/artifacts")
    p.add_argument("--offline-only", action="store_true", help="never call network; use raw parquet only")
    p.add_argument("--run-tag", default=RUN_TAG)
    p.add_argument("--db-path", default=None)
    p.add_argument("--raw-dir", default=None)
    p.add_argument("--cleaned-dir", default=None)
