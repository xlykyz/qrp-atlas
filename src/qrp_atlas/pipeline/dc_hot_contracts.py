"""Formal Pipeline for Eastmoney A-share popularity rank (dc_hot) data."""
...
DC_HOT_TARGET_DATE_POLICY = TargetDatePolicy(
    policy_id="dc_hot_date_or_range_v1",
    description=(
...
def _freshness(context: PipelineRunContext):
    return provider_freshness(context, "tushare_dc_hot")
...
def _translate_provider_error(exc: ContractError) -> ContractError:
    if exc.code in {
        ...
        return ContractError("DC_HOT_API_PARTIAL", exc.detail)
    if exc.code in {"PROVIDER_API_UNAVAILABLE", "PROVIDER_REQUEST_FAILED"}:
        return ContractError("DC_HOT_API_FAILED", exc.detail)
    return exc


def execute_dc_hot_ingest(context: PipelineRunContext) -> BusinessExecution:
    ...
    try:
        ...
        raw, raw_path, empty_dates, rows_read, api_requests = fetch_dc_hot_range(
            context,
            client=client,
        )
        ...
        raise ContractError("DC_HOT_API_FAILED", type(exc).__name__) from exc
    fetched_at = time.monotonic()

    try:
        prepared, clean_path = clean_dc_hot_batch(
            raw,
    ...
    try:
        rows_written, database_seconds = replace_dc_hot_batch(
            context,
            prepared=prepared,
        ...
        raise ContractError("DC_HOT_WRITE_FAILED", type(exc).__name__) from exc
...
DC_HOT_INGEST = register_pipeline(
    PipelineContract(
        pipeline_id="dc_hot_ingest",
        name="Eastmoney A-share popularity rank",
        description=(
            "Fetches Tushare dc_hot records for a bounded date or date range, "
        ...
        executor=execute_dc_hot_ingest,
        target_date_policy=DC_HOT_TARGET_DATE_POLICY,
        parameters=(
            ParameterContract(
                "start_date",
        ...
        inputs=(
            InputContract(
                input_id="tushare_dc_hot",
                kind=InputKind.EXTERNAL_API,
                source="tushare.pro.dc_hot(trade_date=YYYYMMDD, market=A股市场, hot_type=人气榜, is_new=N)",
        ...
        outputs=(
            OutputContract(
                ...
                location="settings.paths.duckdb_path",
                object_name=DC_HOT.name,
                unique_key=DC_HOT.primary_key,
                ...
                completion=CompletionContract(
                    marker="dc_hot target range is queryable after replacement",
                    error_code="DC_HOT_COMPLETION_MISSING",
                    checker=popularity_range_completion(DC_HOT.name, "DC_HOT_COMPLETION_MISSING"),
                ),
                quality_checks=(
                    popularity_unique_key_quality(
                        DC_HOT.name,
                        "DC_HOT_DUPLICATE_KEY",
                    ),
                    popularity_unique_ticker_quality(
                        DC_HOT.name,
                        "DC_HOT_DUPLICATE_TICKER",
                    ),
                ),
                allow_empty=True,
            ),
        ...
        idempotency=IdempotencyContract(
            idempotency_key="dc_hot.(trade_date, snapshot_seq, rank_position)",
        ...
        transaction=TransactionContract(
            ...
            boundary="validated complete dc_hot date snapshots for the resolved date scope",
        ...
        performance=PerformanceBudget(
            ...
            baseline_source="tests/pipeline/test_popularity_contracts.py dc_hot_ingest acceptance path",
        ),
        manual_execution_allowed=True,
    )
)

DC_HOT_UPDATE = DC_HOT_INGEST
DC_HOT_CONTRACTS = (DC_HOT_INGEST,)
