...
def canonical_target_payload(target: StrategyPortfolioTarget) -> dict[str, Any]:
    ...
    positions = [
        ...
        for position in sorted(target.positions, key=lambda item: item.asset_id)
    ]
    return {
...
def _authorization_status(
    ...
    if not authorization_input or PHASE not in authorization_input or V_TRIGGERED not in authorization_input:
        raise ValueError("TASK09_AUTHORIZATION_UNAVAILABLE")
...
def run_task09_daily(
    *,
    ...
    if isinstance(trade_date, datetime) or not isinstance(trade_date, date):
        raise ValueError("TASK09_TARGET_DATE_INVALID")
    if any(not isinstance(value, str) or not value.strip() for value in (rule_version_set_id, parameter_set_id, input_snapshot_id)):
    ...
    provenance = dict(comparison_score_provenance or {})
    for source, target_key in (("score_version", "score_calculation_version"), ("rule_version", "rule_version_set_id"), ("parameter_version", "parameter_set_id")):
        if target_key not in provenance and source in provenance:
            provenance[target_key] = provenance[source]
    provenance.setdefault(TRADE_DATE, trade_date.isoformat())
    ...
    input_provenance = {
        "facts_producer_version": TASK09_FACTS_PRODUCER_VERSION,
        "comparison_score_provenance": provenance,
    ...
    *,
    ...
    try:
        ...
        result_row = connection.execute(
            "SELECT trade_date, result_digest, result_status, strategy_code, strategy_version, rule_version_set_id, parameter_set_id, input_snapshot_id FROM system_b_strategy_result WHERE strategy_run_id=?",
            [strategy_run_id],
        ).fetchone()
        target = connection.execute(
            "SELECT target_identity FROM system_b_strategy_target WHERE strategy_run_id=? AND target_identity=?",
            [strategy_run_id, target_identity],
        ).fetchone()
        if result_row is None or target is None or result_row[0] != trade_date or result_row[1] != result_digest:
            raise ValueError("TASK09_CLOSEOUT_RESULT_MISMATCH")
        completion_status = result_row[2]
        ...
        if existing is not None:
            if tuple(existing) != (completion_identity, result_digest, target_identity, completion_status):
        ...
        connection.execute(
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            [completion_identity, strategy_run_id, trade_date, result_row[3], result_row[4], result_digest, target_identity, target_identity, result_row[5], result_row[6], result_row[7], completion_identity, completion_status, datetime.now(UTC), datetime.now(UTC)],
        )
        connection.execute("COMMIT")
    except Exception:
        connection.execute("ROLLBACK")
