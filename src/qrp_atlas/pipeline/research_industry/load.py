...
def load_report(con: Any, records: list[dict], incremental: bool = False) -> int:
    ...
    df = align_to_schema(
        df,
        RESEARCH_REPORT_INDUSTRY.name,
        fill_missing_optional=True,
        drop_extra=True,
    )
    df = quick_validate(df, RESEARCH_REPORT_INDUSTRY.name, allow_extra=False)
    ...
    insert_sql = (
        f"{insert_sql_keyword} INTO {RESEARCH_REPORT_INDUSTRY.name} ({col_list}) "
