...
用法:
    python -m qrp_atlas.pipeline.research_industry.run --begin 2026-05-01 --end 2026-05-31
    python -m qrp_atlas.pipeline.research_industry.run --begin 2026-05-01 --end 2026-05-31 --incremental
"""
import argparse
import sys
import duckdb

from qrp_atlas.pipeline.research_industry.fetch import fetch_report_list
from qrp_atlas.pipeline.research_industry.fetch_detail import fetch_report_detail
from qrp_atlas.pipeline.research_industry.clean import clean_report, save_raw_csv, save_canonical_csv
from qrp_atlas.pipeline.research_industry.load import load_report
...
def run(begin: str, end: str, incremental: bool = False) -> int:
    ...
    print(f"[research_industry] Pipeline started: {begin} → {end} (mode: {mode})")
    ...
    if not records:
        print("[research_industry] No records fetched, skipping remaining steps")
        print("[research_industry] Pipeline completed successfully")
        return 0
    ...
    try:
        ...
        print(f"[load] {action} {count} records into research_report_industry")

        # Step 7: Verify
        row_count = con.execute(
            "SELECT COUNT(*) FROM research_report_industry WHERE publish_date >= ? AND publish_date <= ?",
            [begin, end]
        ).fetchone()[0]
        print(f"[research_industry] Verified: {row_count} rows in DB")
    finally:
        con.close()
...
def main():
    parser = argparse.ArgumentParser(description="行业研报数据管道")
