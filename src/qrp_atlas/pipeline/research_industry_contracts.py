"""Formal Contract for deterministic industry research-report production.

The two historical industry-research Jobs use the same list, detail, database,
...
The current source pipeline does not calculate industry aggregates and does
not read industry membership or market-price tables. The industry identity is
the classification returned with each Eastmoney research report.
"""
...
from qrp_atlas.contracts import RESEARCH_REPORT_INDUSTRY, align_to_schema, quick_validate
...
from .research_industry.clean import clean_report, write_canonical_csv, write_raw_csv
from .research_industry.config import (
    DETAIL_HEADERS,
    DETAIL_URL_TEMPLATE,
    REPORT_API_URL,
    REPORT_HEADERS,
...
from .research_industry.download_pdf import build_pdf_path
from .research_industry.fetch import (
    IndustryReportFetchReport,
    fetch_report_list_with_report,
)
from .research_industry.fetch_detail import (
    IndustryReportDetailReport,
    fetch_report_detail_with_report,
)
from .research_industry.load import load_report
...
RAW_LOCATION = "settings.paths.raw_dir/research_industry"
CANONICAL_LOCATION = "settings.paths.canonical_dir/research_industry"
PDF_LOCATION = "settings.paths.research_pdfs_dir/research_industry"
REPORT_TABLE = RESEARCH_REPORT_INDUSTRY.name
...
REPORT_CLASSIFICATION_FIELDS: tuple[str, ...] = (
    "industryCode",
    "industryName",
    "indvInduCode",
    "indvInduName",
...
def _parse_date(value: object, field_name: str) -> date:
    ...
    text = _text(value)
    if not text:
        raise ContractError("RESEARCH_INDUSTRY_DATE_INVALID", field_name)
...
def _provider_configuration(context: PipelineRunContext) -> CheckResult:
    ...
    if not valid:
        return CheckResult.failure(
            "research_industry_provider_configuration",
            "RESEARCH_INDUSTRY_PROVIDER_CONFIGURATION_MISSING",
            "Eastmoney industry research-report configuration is incomplete",
        )
    return CheckResult.success(
        "research_industry_provider_configuration",
        list_source=REPORT_API_URL,
        detail_source="Eastmoney industry research-report detail page",
        query_type=REPORT_QTYPE,
        page_size=REPORT_PAGE_SIZE,
        classification_source="provider-supplied industry fields",
    )
...
def _provider_freshness(context: PipelineRunContext, check_id: str) -> CheckResult:
    ...
    return CheckResult.success(
        ...
        classification_rule="industry fields are read from the same report payload; no future membership or price lookup is performed",
        partial_responses="fail closed before any output commit",
    )


def _list_freshness(context: PipelineRunContext) -> CheckResult:
    return _provider_freshness(context, "research_industry_list_freshness")


def _detail_freshness(context: PipelineRunContext) -> CheckResult:
    return _provider_freshness(context, "research_industry_detail_freshness")


def _pdf_freshness(context: PipelineRunContext) -> CheckResult:
    return _provider_freshness(context, "research_industry_pdf_freshness")
...
def _validate_list_records(
    ...
    for index, record in enumerate(records):
        if not isinstance(record, Mapping):
            raise ContractError("RESEARCH_INDUSTRY_PROVIDER_SCHEMA_INVALID", f"record {index} is not an object")
        missing = [field for field in REPORT_REQUIRED_PROVIDER_FIELDS if field not in record]
        if missing:
            raise ContractError(
                "RESEARCH_INDUSTRY_PROVIDER_SCHEMA_INVALID",
                f"record {index} missing {','.join(missing)}",
            )
        info_code = _text(record.get("infoCode"))
        if not info_code:
            raise ContractError("RESEARCH_INDUSTRY_PROVIDER_SCHEMA_INVALID", f"record {index} has empty infoCode")
        if info_code in seen:
            raise ContractError("RESEARCH_INDUSTRY_DUPLICATE_KEY", info_code)
        seen.add(info_code)
        if not _text(record.get("title")):
            raise ContractError("RESEARCH_INDUSTRY_PROVIDER_SCHEMA_INVALID", f"record {index} has empty title")
        publish_date = _parse_date(record.get("publishDate"), f"record[{index}].publishDate")
        if not start_date <= publish_date <= end_date:
            raise ContractError(
                "RESEARCH_INDUSTRY_PROVIDER_WRONG_DATE",
                f"record {index} publishDate {publish_date.isoformat()} is outside target range",
            )


def _classification_label(record: Mapping[str, Any]) -> str:
    for field_name in ("industry_name", "indv_indu_name", "industry_code", "em_industry_code", "indv_indu_code"):
...
def _prepare_rows(cleaned: Sequence[Mapping[str, Any]], start_date: date, end_date: date) -> pd.DataFrame:
    if not cleaned:
        raise ContractError("RESEARCH_INDUSTRY_CLEAN_EMPTY")
    try:
        ...
        raise ContractError("RESEARCH_INDUSTRY_CLEAN_SCHEMA_INVALID", type(exc).__name__) from exc

    for field_name in ("info_code", "title", "publish_date"):
        if frame[field_name].isna().any() or frame[field_name].astype(str).str.strip().eq("").any():
            raise ContractError("RESEARCH_INDUSTRY_CLEAN_SCHEMA_INVALID", field_name)
    if frame["info_code"].duplicated().any():
        raise ContractError("RESEARCH_INDUSTRY_DUPLICATE_KEY")
    publish_dates = pd.to_datetime(frame["publish_date"], errors="coerce").dt.date
    if publish_dates.isna().any():
        raise ContractError("RESEARCH_INDUSTRY_DATE_INVALID", "publish_date")
    if ((publish_dates < start_date) | (publish_dates > end_date)).any():
        raise ContractError("RESEARCH_INDUSTRY_SCOPE_MISMATCH", "publish_date")
    missing_classification = [
        index for index, record in enumerate(frame.to_dict(orient="records"))
        if not _classification_label(record)
    ]
    if missing_classification:
        raise ContractError(
            "RESEARCH_INDUSTRY_CLASSIFICATION_MISSING",
            f"records={missing_classification[:20]}",
        )
    return frame
...
def _file_layout(context: PipelineRunContext) -> _FileLayout:
    ...
    stage_root = Path(
        tempfile.mkdtemp(
            prefix=f"research_industry_{_safe_run_suffix(context.run_id)}_",
    ...
    return _FileLayout(
        ...
        final_raw=settings.paths.raw_dir / "research_industry" / f"{date_tag}.csv",
        final_canonical=settings.paths.canonical_dir / "research_industry" / f"{date_tag}.csv",
        final_pdf_root=settings.paths.research_pdfs_dir / "research_industry",
    )
...
def _download_pdf_to_stage(url: str, destination: Path, control) -> tuple[int, int]:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ContractError("RESEARCH_INDUSTRY_PDF_URL_INVALID")
    ...
    for retry_index in range(PDF_MAX_RETRIES + 1):
        ...
        try:
            ...
            if timeout is not None and timeout <= 0:
                control.check()
                raise TimeoutError("industry research report PDF deadline elapsed")
            requests += 1
            with urllib.request.urlopen(request, timeout=timeout) as response:
                status = getattr(response, "status", None)
                if status is not None and status != 200:
                    raise RuntimeError(f"Eastmoney industry PDF returned HTTP {status}")
                content = response.read()
            control.check()
            if not _valid_pdf_bytes(content):
                raise ValueError("industry research report response is not a valid PDF")
    ...
    raise ContractError(
        "RESEARCH_INDUSTRY_PDF_PROVIDER_FAILED",
...
def _ensure_database_table(connection: duckdb.DuckDBPyConnection) -> None:
    ...
    if present is None:
        raise ContractError("RESEARCH_INDUSTRY_SCHEMA_MISSING", REPORT_TABLE)
    ...
    *,
    ...
    try:
        ...
        if getattr(context.settings.database, "read_only", False):
            raise ContractError("RESEARCH_INDUSTRY_DATABASE_READ_ONLY")
        ...
        if connection is not None and transaction_open:
            connection.execute("ROLLBACK")
        raise
    except Exception as exc:  # noqa: BLE001 - all write failures use a stable code.
        if connection is not None and transaction_open:
            try:
                connection.execute("ROLLBACK")
            except Exception:
                pass
        raise ContractError("RESEARCH_INDUSTRY_WRITE_FAILED", type(exc).__name__) from exc
    finally:
        if connection is not None:
            connection.close()


def _execute_research_industry(context: PipelineRunContext) -> BusinessExecution:
    started = monotonic()
    start_date, end_date = _target_bounds(context)
    list_report: IndustryReportFetchReport
    detail_report: IndustryReportDetailReport
    ...
    try:
        ...
        if not list_report.complete:
            raise ContractError(
                "RESEARCH_INDUSTRY_PROVIDER_PARTIAL",
        ...
        if not raw_records:
            return BusinessExecution.noop(
                "NO_INDUSTRY_REPORTS_IN_TARGET_RANGE",
        ...
        if not detail_report.complete or len(detail_report.records) != len(raw_records):
            raise ContractError(
                "RESEARCH_INDUSTRY_DETAIL_PARTIAL",
        ...
        try:
            ...
            raise ContractError("RESEARCH_INDUSTRY_CLEAN_FAILED", type(exc).__name__) from exc
        _check_context(context)
        ...
        classification_count = len(
            {_classification_label(record) for record in cleaned_records}
        )
        ...
        try:
            ...
            raise ContractError("RESEARCH_INDUSTRY_FILE_STAGE_FAILED", type(exc).__name__) from exc
        ...
        try:
            ...
            raise ContractError("RESEARCH_INDUSTRY_FILE_COMMIT_FAILED", type(exc).__name__) from exc
        _check_context(context)
        database_rows_written = len(cleaned_records) if not incremental else inserted
        output_units = database_rows_written + len(raw_records) + len(cleaned_records) + pdf_report.expected_files
        diagnostics: tuple[PipelineDiagnostic, ...] = ()
        if pdf_report.records_without_url:
            diagnostics = (
                PipelineDiagnostic(
                    code="RESEARCH_INDUSTRY_PDF_URL_MISSING",
                    level=DiagnosticLevel.WARNING,
                    message="some industry reports have no PDF attachment URL",
        ...
        return BusinessExecution.success(
            metrics=PipelineMetrics(
                ...
                assets_processed=classification_count,
                dates_processed=(end_date - start_date).days + 1,
            ...
            outputs=(
                OutputResult(
                    ...
                    rows_written=database_rows_written,
                    ...
                    detail={
                        ...
                        "incremental": incremental,
                        "unique_key": list(RESEARCH_REPORT_INDUSTRY.primary_key),
                        "industry_count": classification_count,
                        "member_count": 0,
                        "classification_semantics": "provider-supplied report fields; no membership or market aggregation",
                    },
                ),
                OutputResult(
                    output_id="research_report_industry_raw_csv",
                ...
                OutputResult(
                    output_id="research_report_industry_canonical_csv",
                ...
                OutputResult(
                    output_id="research_report_industry_pdf",
...
def _db_completion(context: PipelineRunContext) -> CheckResult:
    ...
    try:
        ...
        return CheckResult.failure(
            "research_industry_database_completion",
            "RESEARCH_INDUSTRY_COMPLETION_MISSING",
            "research_report_industry is not queryable after commit",
    ...
    if total <= 0:
        return CheckResult.failure(
            "research_industry_database_completion",
            "RESEARCH_INDUSTRY_COMPLETION_MISSING",
            "target range has no committed industry research-report rows",
            rows=total,
        )
    return CheckResult.success(
        "research_industry_database_completion",
...
def _raw_csv_completion(context: PipelineRunContext) -> CheckResult:
    ...
    try:
        ...
        return CheckResult.failure(
            "research_industry_raw_csv_completion",
            "RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
            "raw industry research-report CSV is missing or invalid",
            exception=type(exc).__name__,
        )
    if rows <= 0 or "" in keys:
        return CheckResult.failure(
            "research_industry_raw_csv_completion",
            "RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
            "raw industry research-report CSV has no valid rows",
            rows=rows,
        )
    return CheckResult.success("research_industry_raw_csv_completion", path=str(layout.final_raw), rows=rows)
...
def _canonical_csv_completion(context: PipelineRunContext) -> CheckResult:
    ...
    try:
        ...
        return CheckResult.failure(
            "research_industry_canonical_csv_completion",
            "RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
            "canonical industry research-report CSV is missing or invalid",
            exception=type(exc).__name__,
        )
    if rows <= 0 or "" in keys:
        return CheckResult.failure(
            "research_industry_canonical_csv_completion",
            "RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
            "canonical industry research-report CSV has no valid rows",
            rows=rows,
        )
    return CheckResult.success(
        "research_industry_canonical_csv_completion",
    ...
    try:
        ...
        try:
            rows = connection.execute(
                f"""
                SELECT info_code, title, industry_name, indv_indu_name,
                       industry_code, em_industry_code, indv_indu_code, publish_date
                FROM {REPORT_TABLE}
        ...
        for (
            ...
            _check_context(context)
            industry_label = _classification_label(
                {
                    "industry_name": industry_name,
        ...
        if missing:
            return CheckResult.failure(
                "research_industry_pdf_quality",
                "RESEARCH_INDUSTRY_PDF_MISSING",
                "one or more industry-report attachment PDFs are missing or invalid",
        ...
        return CheckResult.failure(
            "research_industry_pdf_quality",
            "RESEARCH_INDUSTRY_PDF_CHECK_FAILED",
            "industry research-report PDF quality query failed",
            exception=type(exc).__name__,
        )
...
def _db_quality(context: PipelineRunContext) -> CheckResult:
    ...
    try:
        ...
        return CheckResult.failure(
            "research_industry_database_quality",
            "RESEARCH_INDUSTRY_QUALITY_CHECK_FAILED",
            "industry research-report unique-key quality query failed",
            exception=type(exc).__name__,
        )
    if duplicate is not None:
        return CheckResult.failure(
            "research_industry_database_quality",
            "RESEARCH_INDUSTRY_DUPLICATE_KEY",
            "research_report_industry contains duplicate info_code values",
        )
    return CheckResult.success(
        "research_industry_database_quality",
        unique_key=list(RESEARCH_REPORT_INDUSTRY.primary_key),
...
def _csv_quality(path_selector: str, key: str, check_id: str) -> Any:
    def check(context: PipelineRunContext) -> CheckResult:
        ...
        try:
            ...
            return CheckResult.failure(
                check_id,
                "RESEARCH_INDUSTRY_FILE_QUALITY_FAILED",
                "industry research-report CSV quality check failed",
                exception=type(exc).__name__,
            )
        if rows != len(keys) or "" in keys:
            return CheckResult.failure(
                check_id,
                "RESEARCH_INDUSTRY_DUPLICATE_KEY",
                "industry research-report CSV contains duplicate or empty info_code values",
                rows=rows,
...
def _output_contracts() -> tuple[OutputContract, ...]:
    return (
        OutputContract(
            ...
            unique_key=RESEARCH_REPORT_INDUSTRY.primary_key,
            write_mode=WriteMode.UPSERT,
            target_date_semantics="publish_date is inside the resolved inclusive date range",
            completion=CompletionContract(
                marker="research_report_industry rows are queryable after the database transaction commits",
                error_code="RESEARCH_INDUSTRY_COMPLETION_MISSING",
        ...
        OutputContract(
            ...
            completion=CompletionContract(
                marker="raw industry research-report CSV exists and contains target records",
                error_code="RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
                checker=_raw_csv_completion,
            ),
            quality_checks=(_csv_quality("raw", "infoCode", "research_industry_raw_csv_quality"),),
            allow_empty=True,
        ),
        OutputContract(
            output_id="research_report_industry_canonical_csv",
            ...
            completion=CompletionContract(
                marker="canonical industry research-report CSV exists and contains target records",
                error_code="RESEARCH_INDUSTRY_FILE_COMPLETION_MISSING",
                checker=_canonical_csv_completion,
            ),
            quality_checks=(_csv_quality("canonical", "info_code", "research_industry_canonical_csv_quality"),),
            allow_empty=True,
        ),
        OutputContract(
            output_id="research_report_industry_pdf",
            physical_resource="research_report_files",
            location=PDF_LOCATION,
            object_name="publish-date partitioned industry PDF files",
            unique_key=("info_code",),
            write_mode=WriteMode.UPSERT,
            target_date_semantics="one PDF for each persisted industry-report attachment URL when available",
            completion=CompletionContract(
                marker="all persisted industry-report attachment URLs have valid PDF files",
                error_code="RESEARCH_INDUSTRY_PDF_MISSING",
...
def _input_contracts() -> tuple[InputContract, ...]:
    return (
        InputContract(
            input_id="external_eastmoney_research_industry_list",
            kind=InputKind.EXTERNAL_API,
            source=REPORT_API_URL,
            required_fields=REPORT_REQUIRED_PROVIDER_FIELDS,
            target_date_semantics="publishDate must be inside the explicit resolved inclusive range; industry classification comes from the same provider record",
            missing_error_code="RESEARCH_INDUSTRY_PROVIDER_CONFIGURATION_MISSING",
            structure_check=_provider_configuration,
            freshness=FreshnessContract(
                check_id="research_industry_list_freshness",
                target_date_semantics="complete industry-report list pagination is evaluated for the requested range",
                maximum_lag_trading_days=0,
                non_trading_day_policy=NonTradingDayPolicy.ALLOW_CALENDAR_DATE,
                error_code="RESEARCH_INDUSTRY_PROVIDER_STALE",
                checker=_list_freshness,
            ),
        ),
        InputContract(
            input_id="external_eastmoney_research_industry_detail",
            kind=InputKind.EXTERNAL_API,
            source="Eastmoney industry research-report detail endpoint",
            required_fields=REPORT_DETAIL_PROVIDER_FIELDS,
            target_date_semantics="detail enrichment must succeed for every list record",
            missing_error_code="RESEARCH_INDUSTRY_PROVIDER_CONFIGURATION_MISSING",
            structure_check=_provider_configuration,
            freshness=FreshnessContract(
                check_id="research_industry_detail_freshness",
        ...
        InputContract(
            input_id="external_eastmoney_research_industry_pdf",
            kind=InputKind.EXTERNAL_API,
            source="attachUrl returned by Eastmoney industry research-report detail pages",
            required_fields=("attachUrl",),
            target_date_semantics="each available attachment URL is downloaded and validated before database commit",
            missing_error_code="RESEARCH_INDUSTRY_PROVIDER_CONFIGURATION_MISSING",
            structure_check=_provider_configuration,
            freshness=FreshnessContract(
                check_id="research_industry_pdf_freshness",
                target_date_semantics="PDF content is validated before promotion to the target directory",
                maximum_lag_trading_days=0,
                non_trading_day_policy=NonTradingDayPolicy.ALLOW_CALENDAR_DATE,
                error_code="RESEARCH_INDUSTRY_PROVIDER_STALE",
...
RESEARCH_INDUSTRY_TARGET_DATE_POLICY = TargetDatePolicy(
    policy_id="research_industry_explicit_date_range",
    description=(
...
RESEARCH_INDUSTRY_REPORT_INGEST = register_pipeline(
    PipelineContract(
        pipeline_id="research_industry_report_ingest",
        name="Industry research report ingestion",
        description=(
            "Fetches a complete Eastmoney industry research-report list and detail set for an explicit date range, "
            "validates provider-supplied industry classification, stages raw/canonical CSV and PDF outputs, "
            "then transactionally upserts research_report_industry. It does not aggregate members or prices."
        ),
        contract_version="1.1.0",
        kind=PipelineKind.ATOMIC,
        executor=_execute_research_industry,
        target_date_policy=RESEARCH_INDUSTRY_TARGET_DATE_POLICY,
        parameters=(
            ParameterContract(
                name="start_date",
                parameter_type=ParameterType.DATE,
                description="Inclusive natural start date for industry-report publication.",
                default=None,
            ),
            ParameterContract(
                name="end_date",
                parameter_type=ParameterType.DATE,
                description="Inclusive natural end date for industry-report publication.",
                default=None,
            ),
            ParameterContract(
                name="incremental",
        ...
        transaction=TransactionContract(
            ...
            boundary="one prepared research_report_industry upsert transaction plus staged file promotion",
            failure_visibility=(
        ...
        performance=PerformanceBudget(
            ...
            benchmark_scope=(
                "mocked 50-report industry range with complete list pagination, 50 detail pages, "
                "50 attachment checks, two CSV files, and one temporary-DuckDB transaction"
            ),
            baseline_source=(
                "tests/pipeline/test_research_industry_contracts.py representative mocked 50-report benchmark; "
                "docs/QRP产品蓝图v1.1/09_Pipeline现状事实与迁移边界.md research-industry-0700/1900 facts; "
                "src/qrp_atlas/pipeline/research_industry/config.py request settings"
            ),
        ),
...
RESEARCH_INDUSTRY_CONTRACTS: tuple[PipelineContract, ...] = (RESEARCH_INDUSTRY_REPORT_INGEST,)


__all__ = [
    "RESEARCH_INDUSTRY_CONTRACTS",
    "RESEARCH_INDUSTRY_REPORT_INGEST",
    "RESEARCH_INDUSTRY_TARGET_DATE_POLICY",
