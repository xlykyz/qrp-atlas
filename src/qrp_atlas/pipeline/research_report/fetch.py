"""Eastmoney stock research-report list fetching.
...
class ResearchReportFetchReport:
...
def _request_page(
    ...
    *,
    ...
    if timeout is not None and timeout <= 0:
        ...
        raise TimeoutError("research report request deadline elapsed")
    _check(execution_control)
    with urllib.request.urlopen(req, timeout=timeout) as response:
        status = getattr(response, "status", None)
        if status is not None and status != 200:
            raise RuntimeError(f"Eastmoney returned HTTP {status}")
        payload = json.loads(response.read().decode("utf-8"))
    _check(execution_control)
    if not isinstance(payload, Mapping):
        raise ValueError("research report response must be an object")
    return payload
...
def _reported_total(payload: Mapping[str, Any]) -> int | None:
    ...
    if isinstance(value, bool):
        raise ValueError("research report response total must be a non-negative integer")
    try:
        parsed = int(value)
    except (TypeError, ValueError) as exc:
        raise ValueError("research report response total must be a non-negative integer") from exc
    if parsed < 0:
        raise ValueError("research report response total must be a non-negative integer")
    return parsed
...
def fetch_report_list_with_report(
    ...
    *,
    ...
    A request or response failure makes the report incomplete. The formal
    executor can therefore fail closed before writing any database or file
    output, while the legacy wrapper below can retain its list interface.
    """
        ...
        for retry_index in range(MAX_RETRIES + 1):
            ...
            try:
                ...
                if "data" not in payload:
                    raise ValueError("research report response is missing data")
                raw_records = payload["data"]
                if not isinstance(raw_records, list) or any(
                    not isinstance(record, Mapping) for record in raw_records
                ):
                    raise ValueError("research report response data must be a list of objects")
                ...
                if reported_total is not None and page_total is not None and page_total != reported_total:
                    raise ValueError("research report response total changed between pages")
                if page_total is not None:
                    reported_total = page_total
                break
            except ExecutionControlError:
                raise
            except Exception as exc:  # noqa: BLE001 - provider errors become an incomplete report.
                last_error = exc
...
__all__ = ["ResearchReportFetchReport", "fetch_report_list", "fetch_report_list_with_report"]
