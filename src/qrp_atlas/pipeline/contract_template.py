"""Test-only template showing the minimum formal Pipeline implementation.

It has no deployment selection, performs no I/O, and always returns an explicit
NOOP.  New production pipelines should copy its structure, replace all fixture
checks with real implementations, and add their module to ``contract_catalog``.
"""
...
def execute_template(context) -> BusinessExecution:
    return BusinessExecution.noop(
        "TEMPLATE_NOT_DEPLOYED",
        diagnostics=(),
    )


CONTRACT_TEMPLATE = register_pipeline(
    PipelineContract(
