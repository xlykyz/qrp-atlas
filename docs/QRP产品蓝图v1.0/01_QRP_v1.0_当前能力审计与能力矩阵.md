...
> 任务 04-D 状态更新：2026-07-14，调仓日历、eligibility、Top N、目标权重与 `cross_sectional_momentum_long_only` / `multifactor_long_only` 已完成。任务 04 整体仍为部分完成（04-E 未开始）。
...
1. 横截面基础算子与历史股票池（04-A）、正式动量/市值/财务因子生成（04-B）、行业/市值中性化（04-C）、Top N 与 long-only 横截面策略（04-D）已完成；仍缺因子分析（04-E）、事件指标和残差指标；
2. 财务报表、历史行业、指数成分与 as_of 查询已具备；标准事件时间轴 / corporate_event 仍缺（任务 05）；
3. 组合共享资金、目标权重调仓、A 股现实成交和真实资金曲线已在任务 02 完成；横截面研究到策略闭环（算子/股票池/正式因子/中性化/Top N/策略）已在 04-A～04-D 完成，仍缺完整风险归因与因子分析；
...
| 横截面排序与调仓 | 市值和行情部分已有 | rank/winsorize/z-score、历史股票池（04-A）、正式因子（04-B）、中性化（04-C）、Top N/调仓日历/策略（04-D）已完成 | `cross_sectional_momentum_long_only` / `multifactor_long_only` 已实现 | 组合账户、目标权重和调仓底座已完成 | 组合结果可读，缺 IC/分组等研究输出 | 🟡 | 04-A～04-D 完成到策略闭环；04-E 因子分析仍缺 |
| 多因子 long-only | 财务/行业/指数历史与 as_of 查询已具备（事件仍缺） | 正式因子（04-B）、中性化（04-C）、`multifactor_long_only`（04-D）已完成 | 已实现 long-only 代表策略 | 组合调仓底座已完成 | 缺 IC/分组与暴露归因 | 🟡 | 策略已完成；04-E 研究分析仍缺 |
...
| 横截面动量 | 行情、市值字段；目标权重与组合调仓底座已完成；rank/历史股票池（04-A）、动量因子（04-B）、中性化（04-C）、策略（04-D）已完成 | 因子分析与更完整研究输出 | `cross_sectional_momentum_long_only` |
| 多因子 | daily_basic + 财务/行业/指数历史与 as_of 查询（任务 03 完成）；组合账户底座已完成；正式因子（04-B）、中性化（04-C）、策略（04-D）已完成 | 因子分析、事件数据 | `multifactor_long_only` |
...
- 因子分析 IC/分组收益等（04-A～04-D 已完成 rank/标准化/历史股票池、正式因子、中性化、Top N 与横截面策略）；
