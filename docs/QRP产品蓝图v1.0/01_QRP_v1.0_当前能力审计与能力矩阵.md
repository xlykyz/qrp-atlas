...
> 任务 04-C 状态更新：2026-07-14，行业/市值暴露面板与横截面中性化已完成。任务 04 整体仍为部分完成（04-D～04-E 未开始）。
...
1. 横截面基础算子与历史股票池（04-A）、正式动量/市值/财务因子生成（04-B）、行业/市值中性化（04-C）已完成；仍缺 Top N、横截面策略、事件指标和残差指标；
2. 财务报表、历史行业、指数成分与 as_of 查询已具备；标准事件时间轴 / corporate_event 仍缺（任务 05）；
3. 组合共享资金、目标权重调仓、A 股现实成交和真实资金曲线已在任务 02 完成；横截面研究底座（算子/股票池/正式因子/中性化）已在 04-A/04-B/04-C 完成，仍缺 Top N/策略闭环与更完整风险归因；
...
| 横截面排序与调仓 | 市值和行情部分已有 | rank/winsorize/z-score、历史股票池（04-A）、正式因子（04-B）与行业/市值中性化（04-C）已完成 | 未实现正式横截面策略 | 组合账户、目标权重和调仓底座已完成 | 组合结果可读，缺横截面研究输出 | 🟡 | 04-A/04-B/04-C 完成算子、股票池、因子与中性化；仍缺 Top N、调仓周期和横截面策略 |
| 多因子 long-only | 财务/行业/指数历史与 as_of 查询已具备（事件仍缺） | 正式因子（04-B）与行业/市值中性化（04-C）已完成 | 未实现 | 组合调仓底座已完成，缺多因子研究链路 | 缺归因 | 🟡 | v1.0 必须完成代表实现 |
...
| 横截面动量 | 行情、市值字段；目标权重与组合调仓底座已完成；rank/历史股票池（04-A）、动量因子（04-B）与中性化（04-C）已完成 | Top N、调仓周期和横截面策略 | `cross_sectional_momentum_long_only` |
| 多因子 | daily_basic + 财务/行业/指数历史与 as_of 查询（任务 03 完成）；组合账户底座已完成；正式因子（04-B）与行业/市值中性化（04-C）已完成 | 多因子横截面策略、事件数据 | `multifactor_long_only` |
...
- Top N 与横截面策略（04-A/04-B/04-C 已完成 rank/标准化/历史股票池、正式因子与行业/市值中性化）；
