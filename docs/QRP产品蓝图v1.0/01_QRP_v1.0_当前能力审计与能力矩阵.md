...
> 任务 02 状态更新：2026-07-13，组合共享资金、A 股现实撮合、真实资金曲线和 `suspend_d` 停牌字段接入已完成（PR #5，待最终人工合并）。
> 任务 03 状态更新：2026-07-14，03-A PIT 版本选择器（PR #7）与 03-B Tushare 数据底座（PR #8）已合入 main；任务 03 仍为部分完成。
...
2. 财务报表、历史行业与指数成分底座已入库，但仍缺 PIT 查询服务与标准事件时间轴；
3. 组合共享资金、目标权重调仓、A 股现实成交和真实资金曲线已在任务 02 完成；仍缺横截面/多因子研究链路与更完整风险归因；
...
| 多因子 long-only | 财务/行业/指数历史底座已入库（PIT 查询与事件仍缺） | 缺因子与中性化 | 未实现 | 组合调仓底座已完成，缺多因子研究链路 | 缺归因 | ⛔ | v1.0 必须完成代表实现 |
...
| 多因子 | daily_basic + 财务/行业/指数历史底座（03-A/03-B 已合入）；组合账户底座已完成 | PIT 查询服务、因子与中性化、多因子横截面策略 | `multifactor_long_only` |
