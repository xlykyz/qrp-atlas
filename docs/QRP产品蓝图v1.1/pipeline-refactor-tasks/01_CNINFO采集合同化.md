...
当前源码显示的核心业务能力是：对显式目标自然日或明确日期范围，从 Eastmoney 获取机构调研记录，完成分页、清洗、契约校验和幂等入库。默认应收口为一条正式业务 Contract；只有源码和业务规则证明存在与调度时间无关的不同算法、输入或输出时，才允许拆分。
...
2. Contract 对一个显式目标自然日或明确日期范围执行采集。目标日期/范围属于业务调用参数或 `PipelineInvocation`，不得由 Hermes wrapper 语义决定。
