...
| `3fdbd779c4da` / irm_qa 每五分钟增量更新 | `*/5 8-21 * * *`；`irm_qa_daily_update.sh` -> `irm_qa.run` | 脚本 `cd` 项目根；无 LLM | shell 前后读 `irm_interaction_qa` COUNT；写同表；全景网 P5W API（单请求 TO=15 秒，`src/qrp_atlas/pipeline/irm_qa/fetch.py:30-54`） | `INSERT OR IGNORE` 按 `pid`（`src/qrp_atlas/pipeline/irm_qa/load.py:51-61`）；无显式 transaction。Hermes 同 job-id guard 防自重叠，但脚本无独立锁；与其他 `quant.db` writer 仍可能并发。前/后验证读锁失败时脚本 `exit 0`（`~/.hermes/scripts/irm_qa_daily_update.sh:35-52`），故 RC 不总代表完成验证；核心 runner 非零会 `exit 1`（`:41-45`）。 |
