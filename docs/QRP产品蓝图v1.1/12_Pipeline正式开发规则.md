...
`qrp_atlas.pipeline.examples.contract_template` 是无 I/O、无真实凭据、无部署选择的参考模板。它只演示 Contract、executor、NOOP、测试和 runtime 结果接口，不能被当作生产数据任务，也绝不加入默认 catalog。当前默认 catalog 显式导入 `qrp_atlas.pipeline.market_data_contracts`、`qrp_atlas.pipeline.etf_daily_contracts`、`qrp_atlas.pipeline.etf_adj_factor_contracts`、`qrp_atlas.pipeline.index_basic_contracts`、`qrp_atlas.pipeline.stock_basic_contracts`、`qrp_atlas.pipeline.limit_step_contracts`、`qrp_atlas.pipeline.ths_daily_contracts`、`qrp_atlas.pipeline.stk_high_shock_contracts`、`qrp_atlas.pipeline.cninfo_contracts`、`qrp_atlas.pipeline.irm_qa_contracts`、`qrp_atlas.pipeline.membership_contracts`、`qrp_atlas.pipeline.pit_fundamentals_contracts`、`qrp_atlas.pipeline.research_report_contracts` 和 `qrp_atlas.pipeline.research_industry_contracts`，共包含二十二条通过正式验收的 Contract：六条基础市场数据 Contract、两个 ETF 快照 Contract、`index_basic_update`、`stock_basic_update`、三个 Tushare 快照 Contract（`limit_step_ingest`、`ths_daily_ingest`、`stk_high_shock_ingest`）、`cninfo_research_visit_ingest`、`irm_qa_incremental`、`industry_membership_ingest`、`index_component_ingest`、`pit_backfill`、`fundamentals_ingest`、`earnings_forecast_ingest`、`research_stock_report_ingest` 以及 `research_industry_report_ingest`。ETF 与 Tushare Contract 共享 `contracts/fields.py`、`contracts/mappings.py` 和 `contracts/schema.py` 的公共 SSOT，并分别写入 canonical `quant.db` 的 `etf_daily`、`etf_adj_factor`、`limit_step`、`ths_daily` 和 `stk_high_shock` 表。三个 Tushare 快照 Contract 按请求日期逐日读取 provider 响应，只接受完整日期快照，不提供 `ts_code`/`nums` 局部过滤；使用显式日期或日期范围、逐请求日期校验、`quant_db_writer`、单次事务和完整非空日期替换语义。provider 返回 `None` 或已有目标数据对应的空响应时 fail-closed；目标日期原本无数据的空响应不执行删除。Tushare 没有可靠的 total/page 证据，结果保留这一完整性边界。两个研究报告 Contract 使用显式日期范围、完整列表/详情响应、数据库事务、run-scoped CSV/PDF staging、`quant_db_writer` 和 `overlap_policy=FORBID`；行业报告分类直接使用 provider 字段，不读取行业成员历史或行情表，也不执行成员/价格聚合；历史晨间和晚间 Job 只是同一业务能力的调度事实。IRM 使用 P5W 最新 feed、`pid` 幂等追加、独立 `irm_qa.duckdb`（`irm_qa_db` 资源、`irm_qa_writer` 锁）和 `overlap_policy=FORBID`；成员关系 Contract 使用显式范围、`revision_id` 幂等追加、`quant_db_writer` 和 `overlap_policy=FORBID`。其他既有 Pipeline 仍不会被默认 CLI 发现或进入 QRP 正式调度。源码 catalog 可发现不等于部署启用：本仓库没有为这些 Contract 新增部署选择、Production Definition、systemd 或 Hermes 变更。
