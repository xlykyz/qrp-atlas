...
- 高度池：连续两次收盘涨停进入自然高度结构；任意连续七个实际观察日内至少三次收盘涨停也可建立结构。结构保存累计涨停次数 `n`、最大已完成断板长度 `m`、反包次数 `i`，断板第五个实际观察日退出。
- 容量池：仅 `ACTIVE` 股票可入池，且满足当日成交额排名前 100、五日实际观察日平均成交额排名前 100、流通市值不低于 300 亿元之一。一字涨停当日退出。
- 辨识度池：轮次累计涨幅不低于 30% 且轮次排名前 30，或五日/十日区间涨幅排名前 30。首次入池必须来自当日高度池或容量池；已入池股票在轮次观察仍持续时保留，三项条件均失效且无轮次观察后退出。
...
当前正式规则登记未给出“缩量”的定量阈值。容量判定不会擅自设置比例或默认值，输出指标中保留 `shrink_volume: NOT_CONFIGURED`，因此该条规则需要后续正式裁决后才能启用；其它明确规则不受影响。
