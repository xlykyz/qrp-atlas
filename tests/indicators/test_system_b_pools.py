...
def test_height_two_board_entry_and_break_day_five_exit():
    dates = pd.bdate_range("2026-01-01", periods=8)
    frame = _frame([
        {"asset_id": "A", "trade_date": dates[0], "is_limit_up": False},
        {"asset_id": "A", "trade_date": dates[1], "is_limit_up": True},
        {"asset_id": "A", "trade_date": dates[2], "is_limit_up": True},
        {"asset_id": "A", "trade_date": dates[3], "is_limit_up": False},
        {"asset_id": "A", "trade_date": dates[4], "is_limit_up": False},
        {"asset_id": "A", "trade_date": dates[5], "is_limit_up": False},
        {"asset_id": "A", "trade_date": dates[6], "is_limit_up": False},
        {"asset_id": "A", "trade_date": dates[7], "is_limit_up": False},
    ])
    result = evaluate_height(build_common_features(frame)).membership
    active_rows = result.loc[result["membership_state"] == IN_POOL]
    assert not active_rows.empty
    assert active_rows.iloc[0]["entry_date"] == dates[2]
